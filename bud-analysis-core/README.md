...
│   ├── docs/                     (.md specs — see below)
│   │   ├── core/                 (library modules)
│   │   └── task-template/        (the 4 commands + 1 config every task copies)
│   ├── core/       (the Python package; snake_case)
│   │   └── __init__.py
│   ├── viewer/                  (Flask app: interactive predicted-vs-true explorer — see viewer/README.md)
│   └── tests/
├── ripeness/                     ← task project (separate, created when you start a task)
└── defects/                      ← task project (separate)
```
...
| Module | Purpose |
|---|---|
| [`schemas.py`](docs/core/schemas.md) | `HeadSpec` + `TrainResult` — the two dataclasses crossing the core ↔ task boundary |
| [`run_context.py`](docs/core/run_context.md) | `RunContext` — single source of truth for every output path |
...
Three command scripts + one config module per task project. When you create a new task project (e.g. `<workspace>/ripeness/`), copy these `.py` files into it, edit `config.py`, leave the rest alone.
...
│   ├── info.json             # run manifest: identity + backbone + abs data_dir + stats + frozen training config
...
| Where | Convention |
|---|---|
| CSV column names on disk | camelCase (`fileName`, `flowerID`, `forkID`, `viewType`) — except `class`, which stays as `class` |
...
2. Pick a module from `core/` or `task/` and read its `.md`. Each doc has Purpose, Public API, Inputs, Outputs, Notes — same shape everywhere.
3. The task ↔ library contract is `core/schemas.md`. Read that one first if you're trying to understand how a task plugs into the library.
