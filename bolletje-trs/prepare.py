...
def main(config, data_dir, cultivar, backbone=None, views=None,
    ...
    df = data.run(
        ...
        # Bolletje is ~2% of flowers. Stratify by class so the rare positive is
        # forced into val AND test (needs >= 3 positive flowers); a plain random
        # split could leave val/test with zero positives, breaking PR-AUC/F1.
        stratify_key="class",
