"""train.py — the bolletje training loop (the only task file with real logic).

Binary-defect twin of `ripeness-trs/train.py`. The pipeline is identical — frozen
DINOv3 embeddings → sigmoid head → mil_mean over views → Optuna study → keep best per
variant → comparison plot — but the target is {0, 1} (bolletje / no bolletje) and the
sigmoid output is read as P(bolletje). Two things differ from the ripeness loop, both
additive so the shared plotting/export machinery is untouched:
  * `bce` is a selectable loss (log-loss on the probability); `mse` stays available as
    the Brier score. Regression metrics (rmse/mae) are still computed and drive the
    scatter, which now reads as a calibration plot (true ∈ {0, 1}).
  * Classification metrics (accuracy, precision, recall, F1 at 0.5, ROC-AUC, PR-AUC) are
    added to `metrics.json`, and model selection minimises `(1 - val_pr_auc)
    + β·val_view_range` — PR-AUC, not RMSE, because bolletje is expected to be the rare
    class and PR-AUC is the honest measure of separating a rare positive.
...
override the frozen hparams per run.
"""
...
def _weighted_bce(pred, target, pos_weight):
    """Binary cross-entropy on sigmoid outputs with the positive class up-weighted.

    `pred` is P(bolletje) in [0, 1] (already sigmoid'd), `target` is 0/1, `pos_weight`
    multiplies the positive term (n_neg/n_pos) so the ~2% positives aren't ignored.
    `pred` is clamped off 0/1 so log() never blows up.
    """
    p = pred.clamp(1e-6, 1.0 - 1e-6)
    loss = -(pos_weight * target * torch.log(p) + (1.0 - target) * torch.log(1.0 - p))
    return loss.mean()


def _loss_fn(name, pos_weight=1.0):
    if name == "bce":
        return lambda pred, target: _weighted_bce(pred, target, pos_weight)
    return nn.HuberLoss() if name == "huber" else nn.MSELoss()


def _pr_auc(pred, target):
    """Average-precision (area under precision-recall) of `pred` vs binary `target`.

    Returns None when only one class is present in `target` (PR-AUC is undefined then),
    so callers can fall back. `pred` is P(bolletje) in [0, 1]; `target` is 0/1.
    """
    ...
    if len(np.unique(t)) < 2:
        return None
    return float(average_precision_score(t, np.asarray(pred).reshape(-1)))


def _clf_metrics(pred, target, threshold=0.5):
    """Binary-classification metrics for P(bolletje) vs 0/1 target, thresholded.

    accuracy/precision/recall/f1 at `threshold` (0.5), plus threshold-free ROC-AUC and
    PR-AUC (None if `target` has a single class). Predictions are the flower-level
    mil_mean probabilities, so this scores the deployed decision, not per-view outputs.
    """
    ...
    both = len(np.unique(t)) >= 2
    return {
        "accuracy": float(accuracy_score(t, yhat)),
        "precision": float(precision_score(t, yhat, zero_division=0)),
        "recall": float(recall_score(t, yhat, zero_division=0)),
        "f1": float(f1_score(t, yhat, zero_division=0)),
        "roc_auc": float(roc_auc_score(t, p)) if both else None,
        "pr_auc": _pr_auc(p, t),
        "threshold": threshold,
...
def _objective(trial, ctx, index, emb, settings, scratch):
    ...
    # Class weighting for the ~2% positive rate: pos_weight = n_neg / n_pos on the train
    # split, so the rare bolletje flowers aren't drowned out. Applied to `bce` only
    # (`mse`/Brier ignores it); 1.0 if the split somehow has no positives.
    n_pos = float((y[train_mask] == 1).sum())
    pos_weight = float((y[train_mask] == 0).sum()) / n_pos if n_pos > 0 else 1.0
    loss_fn = _loss_fn(loss_name, pos_weight)
    ...
    with torch.no_grad():
        ...
        pooled_val, pv_val = _forward(head, X[val_mask], return_views=True)
    ...
    # Robustness-aware selection: separation + β · view disagreement (lower is better).
    # Primary term is (1 - val PR-AUC): bolletje is the rare class, so PR-AUC measures
    # separating the positive far better than RMSE. If val holds a single class (tiny/
    # imbalanced split), PR-AUC is undefined → fall back to val_rmse so selection never
    # breaks.
    val_pr_auc = _pr_auc(pooled_val.squeeze(1).numpy(), y[val_mask].squeeze(1).numpy())
    separation = (1.0 - val_pr_auc) if val_pr_auc is not None else history[-1]["val_rmse"]
    selection_score = separation + beta * val_view_range
    clf = _clf_metrics(pt.squeeze(1).numpy(), y[test_mask].squeeze(1).numpy())
    ...
    metrics = {
        ...
        **clf,                       # accuracy/precision/recall/f1/roc_auc/pr_auc + counts
