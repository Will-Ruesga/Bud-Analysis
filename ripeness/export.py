...
def _parse_heads(spec_list):
    """['ripeness:auto', 'defects:mil_mean'] → [('ripeness','auto'), ...]."""
    out = []
    for item in spec_list:
        task, _, agg = item.partition(":")
        out.append((task, agg or "auto"))
    return out


def main(run_dir, aggregator=None, heads=None, output_path=None):
    """Resolve the requested head(s) for a prepared run and export them to one ONNX file."""
    ctx = RunContext.from_info_json(run_dir)
    if heads is None:
        heads = [(ctx.task, aggregator or "auto")]
...
if __name__ == "__main__":
    ...
    g.add_argument("--aggregator")
    g.add_argument("--heads", nargs="+")
    ap.add_argument("--output_path")
    a = ap.parse_args()
    heads = _parse_heads(a.heads) if a.heads else None
    print(main(a.run, aggregator=a.aggregator, heads=heads, output_path=a.output_path))
